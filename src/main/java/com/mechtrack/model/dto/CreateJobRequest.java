...
public class CreateJobRequest {
    ...
    @Schema(description = "Name of the customer", example = "John Smith", required = true)
    ...
    @Schema(description = "Car model and year", example = "Toyota Camry 2018", required = true)
    private String carModel;
    
    @NotBlank(message = "Description is required")
    @Size(min = 5, max = 1000, message = "Description must be between 5 and 1000 characters")
    @Pattern(regexp = "^[a-zA-Z0-9\\s\\-_\\.,!?\\(\\)]+$", message = "Description contains invalid characters")
    @Schema(description = "Detailed description of the repair work", example = "Oil change and brake inspection", required = true)
    private String description;
    
    @NotNull(message = "Date is required")
    @PastOrPresent(message = "Date cannot be in the future")
    @Schema(description = "Date when the job was performed", example = "2023-12-01", required = true)
    ...
    @Schema(description = "Total income from the job", example = "150.00", required = true)
